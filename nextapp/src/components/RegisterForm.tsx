...
const RegisterForm: React.FC<RegisterPageProps> = ({ className, SetRegister, ...props }: RegisterPageProps) => {

    const mutation = useMutation<{ user: UserDoc; success: boolean } | undefined, Error, Registerinformation>({
        mutationFn: (registerInfo: Registerinformation) => registerUser(registerInfo),
        onSuccess: (data) => {
            if (SetRegister) {
                SetRegister(true);
            }
    ...
    return (<> 
        <Card className="w-[350px]">
            ...
            <CardFooter className="flex justify-between">
                <LabelCheckBox LabelContent={"Accept terms and conditions"}></LabelCheckBox> <Button onClick={handleSubmit}>Register</Button>
            </CardFooter>
        </Card >
